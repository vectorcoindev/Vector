...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("vector://"))
    {
        uri.replace(0, 12, "vector:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Vector.lnk";
}
...
{
    return GetAutostartDir() / "vector.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Vector-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  vector-qt [" + tr("command-line options") + "]                     " + "\n";
