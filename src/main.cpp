...
#if defined(NDEBUG)
# error "Vector cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Vector Signed Message:\n";
...
{
    int64_t PreMine = 15000000 * COIN;
    if(pindexBest->nHeight == 0){return PreMine;} else {return 1*COIN;}
}
...
{
    string strBlockFn = strprintf("VEC%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile VECdat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && VECdat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(VECdat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), VECdat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
